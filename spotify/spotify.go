...
func accessToken() (string, error) {
    ...
    if resp.StatusCode != 200 {
        ...
        return "", fmt.Errorf("failed to get access token: received non-200 status code %d", resp.StatusCode)
    }
...
func request(endpoint string) (int, string, error) {
	...
	fmt.Println("Access Token: ", bearer)  // Print out the token
...
func TrackInfo(url string) (*Track, error) {
	...
	// Fetch first artist
	if firstArtist := gjson.Get(jsonResponse, "artists.0.name").String(); firstArtist != "" {
		allArtists = append(allArtists, firstArtist)
	}
	// Fetch other artists
	if artists := gjson.Get(jsonResponse, "artists").Array(); len(artists) > 1 {
		for _, artist := range artists[1:] {
			if name := artist.Get("name").String(); name != "" {
	...
	durationInSeconds := int(gjson.Get(jsonResponse, "duration_ms").Int()) / 1000

	// Fetch the track title and album name correctly
	title := gjson.Get(jsonResponse, "name").String()
	album := gjson.Get(jsonResponse, "album.name").String()
	
	track := &Track{
		Title:    title,
		Artist:   gjson.Get(jsonResponse, "artists.0.name").String(),
		Artists:  allArtists,
