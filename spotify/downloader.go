...
// downloadYTaudio downloads the audio of a YouTube video using yt-dlp
func downloadYTaudio(id, path, filePath string) error {
	...
	if !dir.IsDir() {
		return errors.New("the path is not valid (not a directory)")
	}
	...
	ytdlpCmd := exec.Command("yt-dlp",
		...
		"-o", filePath,         // Set output file path
		"https://www.youtube.com/watch?v="+id, // Video URL
	)

	// Run the command and capture output
	output, err := ytdlpCmd.CombinedOutput()
	if err != nil {
		fmt.Printf("Error downloading audio for video ID %s\n", id)
		fmt.Println("Make sure yt-dlp and ffmpeg are installed.")
		fmt.Println("Command attempted:", ytdlpCmd.String())
		fmt.Println("Error details:", string(output))
		return err
	}
